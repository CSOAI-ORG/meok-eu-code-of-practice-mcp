        ...
        if (customerEmail && process.env.RESEND_API_KEY) {
          try {
            const resendResponse = await fetch('https://api.resend.com/v1/emails', {
              ...
              body: JSON.stringify({
                from: 'PlantHire.ai <welcome@planthire.ai>',
                to: [customerEmail],
                subject: `Welcome to PlantHire.ai ${tier === 'enterprise' ? 'Enterprise' : 'Pro'}`,
                html: `
                  <h2>Welcome to PlantHire.ai!</h2>
                  <p>Thank you for subscribing to our ${tier} plan.</p>
                  <p>Your access is now active. Visit <a href="https://planthire.ai">planthire.ai</a> to get started.</p>
