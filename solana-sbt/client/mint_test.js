...
async function main() {
    ...
    for (let i = 0; i < archetypes.length; i++) {
        ...
        try {
            const sig = await client.mint(issuer, issuer.publicKey, // Mint to self for testing
            SbtType.CharacterGenesis, BigInt(i + 1), metadataUri, 0, // risk tier
    ...
    try {
        const sig = await client.mint(issuer, issuer.publicKey, SbtType.AgentIdentity, BigInt(1), "ipfs://meok-agent-001", 2, // High risk tier
        BigInt(0));
    ...
    try {
        const sig = await client.mint(issuer, issuer.publicKey, SbtType.VerifierReputation, BigInt(1), "ipfs://meok-verifier-001", 0, BigInt(0));
