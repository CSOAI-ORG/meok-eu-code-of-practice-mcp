...
export class RealtimeChat {
  ...
  async init() {
    try {
      ...
      if (error) {
        console.error('[RealtimeChat] Error getting token:', error);
        throw error;
      }
      
      if (!data.client_secret?.value) {
        console.error('[RealtimeChat] No ephemeral token in response:', data);
      ...
      if (!sdpResponse.ok) {
        ...
        console.error('[RealtimeChat] SDP exchange failed:', sdpResponse.status, errorText);
      ...
      console.log('[RealtimeChat] Audio recording started');
