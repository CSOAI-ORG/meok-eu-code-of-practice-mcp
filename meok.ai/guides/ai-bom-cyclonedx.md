# How to generate an AI Bill of Materials (AI BOM)

**Regulation:** EU AI Act (EU AI Act Art 13 + NIST AI 600-1)
**MCP package:** [ai-bom-mcp](https://pypi.org/project/ai-bom-mcp/)
...
Generating CycloneDX 1.6 AI BOM for an AI system — model, datasets, dependencies, training compute.

This guide walks through the practical steps to implement this using the MEOK `ai-bom-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The EU AI Act (EU AI Act Art 13 + NIST AI 600-1) is enforced as of EU AI Act Art 13(3)(b) + NIST AI 600-1. Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
Your agent will call the `ai-bom-mcp` tool, which references the EU AI Act EU AI Act Art 13 + NIST AI 600-1 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["EU AI Act Art 13 + NIST AI 600-1", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in EU AI Act Art 13 + NIST AI 600-1."
}
```
...
```python
from ai_bom_mcp import build_evidence_pack
...
- MEOK MCP: [ai-bom-mcp](https://pypi.org/project/ai-bom-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: EU AI Act Art 13(3)(b) + NIST AI 600-1
