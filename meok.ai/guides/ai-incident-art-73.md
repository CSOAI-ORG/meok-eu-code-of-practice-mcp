# How to file an Article 73 serious incident report

**Regulation:** EU AI Act (Article 73)
**MCP package:** [ai-incident-reporting-mcp](https://pypi.org/project/ai-incident-reporting-mcp/)
...
Filing the post-market incident report for a serious incident involving a high-risk AI system.

This guide walks through the practical steps to implement this using the MEOK `ai-incident-reporting-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The EU AI Act (Article 73) is enforced as of Article 73(2). Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install ai-incident-reporting-mcp
```
...
Your agent will call the `ai-incident-reporting-mcp` tool, which references the EU AI Act Article 73 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 73", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 73."
}
```
...
```python
from ai_incident_reporting_mcp import build_evidence_pack
...
- MEOK MCP: [ai-incident-reporting-mcp](https://pypi.org/project/ai-incident-reporting-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: Article 73(2)
