# How to map DORA obligations to NIS2 requirements

**Regulation:** DORA (Article 1 + Annex)
**MCP package:** [dora-nis2-crosswalk-mcp](https://pypi.org/project/dora-nis2-crosswalk-mcp/)
...
Building a single compliance matrix for a financial entity subject to both DORA and NIS2.

This guide walks through the practical steps to implement this using the MEOK `dora-nis2-crosswalk-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The DORA (Article 1 + Annex) is enforced as of DORA + NIS2. Non-compliance is a fine of up to €5M or 2.5-3% of global annual turnover (whichever is higher) for DORA. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install dora-nis2-crosswalk-mcp
```
...
Your agent will call the `dora-nis2-crosswalk-mcp` tool, which references the DORA Article 1 + Annex text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 1 + Annex", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 1 + Annex."
}
```
...
```python
from dora_nis2_crosswalk_mcp import build_evidence_pack
...
- MEOK MCP: [dora-nis2-crosswalk-mcp](https://pypi.org/project/dora-nis2-crosswalk-mcp/)
