# How to satisfy Article 10 data governance for bias

**Regulation:** EU AI Act (Article 10)
**MCP package:** [bias-detection-mcp](https://pypi.org/project/bias-detection-mcp/)
...
Detecting and reporting demographic bias in training data and model output.

This guide walks through the practical steps to implement this using the MEOK `bias-detection-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The EU AI Act (Article 10) is enforced as of Article 10(2)(f). Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install bias-detection-mcp
```
...
Your agent will call the `bias-detection-mcp` tool, which references the EU AI Act Article 10 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 10", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 10."
}
```
...
```python
from bias_detection_mcp import build_evidence_pack
...
- MEOK MCP: [bias-detection-mcp](https://pypi.org/project/bias-detection-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: Article 10(2)(f)
