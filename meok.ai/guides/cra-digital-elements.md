# How to determine if your product has 'digital elements'

**Regulation:** CRA (Article 3(1))
**MCP package:** [cra-compliance-mcp](https://pypi.org/project/cra-compliance-mcp/)
...
What 'products with digital elements' covers — including remote data processing solutions.

This guide walks through the practical steps to implement this using the MEOK `cra-compliance-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The CRA (Article 3(1)) is enforced as of Article 3(1). Non-compliance is a fine of up to €5M or 2.5-3% of global annual turnover (whichever is higher) for CRA. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install cra-compliance-mcp
```
...
Your agent will call the `cra-compliance-mcp` tool, which references the CRA Article 3(1) text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 3(1)", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 3(1)."
}
```
...
```python
from cra_compliance_mcp import build_evidence_pack
...
- MEOK MCP: [cra-compliance-mcp](https://pypi.org/project/cra-compliance-mcp/)
