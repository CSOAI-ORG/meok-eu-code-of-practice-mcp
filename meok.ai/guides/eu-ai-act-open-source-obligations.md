# How to do provider vs deployer obligations for an open-source model
...
When you fine-tune an OSS model and deploy it — which obligations apply?
...
The EU AI Act (Article 25) is enforced as of Article 25. Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
Your agent will call the `eu-ai-act-compliance-mcp` tool, which references the EU AI Act Article 25 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 25", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 25."
}
```
