# How to determine if your entity is 'essential' or 'important'

**Regulation:** NIS2 (Article 3 + Annex I/II)
**MCP package:** [nis2-compliance-mcp](https://pypi.org/project/nis2-compliance-mcp/)
...
Sector-based classification of essential entities (energy, transport, banking) vs important entities.

This guide walks through the practical steps to implement this using the MEOK `nis2-compliance-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The NIS2 (Article 3 + Annex I/II) is enforced as of Article 3(1) + Annex. Non-compliance is a fine of up to €10M or 2.5-3% of global annual turnover (whichever is higher) for NIS2. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install nis2-compliance-mcp
```
...
Your agent will call the `nis2-compliance-mcp` tool, which references the NIS2 Article 3 + Annex I/II text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 3 + Annex I/II", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 3 + Annex I/II."
}
```
...
```python
from nis2_compliance_mcp import build_evidence_pack
...
- MEOK MCP: [nis2-compliance-mcp](https://pypi.org/project/nis2-compliance-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: Article 3(1) + Annex
