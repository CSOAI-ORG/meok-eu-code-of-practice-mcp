# How to verify hardware root-of-trust on a fleet

**Regulation:** Hardware (NIST SP 800-155)
**MCP package:** [meok-attestation-verify](https://pypi.org/project/meok-attestation-verify/)
**Install:** `pip install meok-attestation-verify`
**Time to implement:** ~30 minutes
**Skill level:** Senior engineer / compliance lead

## Context

Running TPM 2.0 / Apple Secure Enclave / Android Keystore attestation across a device fleet.

This guide walks through the practical steps to implement this using the MEOK `meok-attestation-verify` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The Hardware (NIST SP 800-155) is enforced as of NIST SP 800-155. Non-compliance is a fine of up to €5M or 2.5-3% of global annual turnover (whichever is higher) for Hardware. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install meok-attestation-verify
```
...
Your agent will call the `meok-attestation-verify` tool, which references the Hardware NIST SP 800-155 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["NIST SP 800-155", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in NIST SP 800-155."
}
```
...
```python
from meok_attestation_verify import build_evidence_pack
...
- MEOK MCP: [meok-attestation-verify](https://pypi.org/project/meok-attestation-verify/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: NIST SP 800-155
