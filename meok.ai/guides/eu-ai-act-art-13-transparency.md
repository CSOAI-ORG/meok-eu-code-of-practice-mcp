# How to satisfy Article 13 transparency for high-risk AI

**Regulation:** EU AI Act (Article 13(3)(b))
**MCP package:** [explainability-report-mcp](https://pypi.org/project/explainability-report-mcp/)
...
Generating per-decision explanation reports for a loan-approval model.

This guide walks through the practical steps to implement this using the MEOK `explainability-report-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The EU AI Act (Article 13(3)(b)) is enforced as of Article 13(3)(b). Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.
...
```bash
uv pip install explainability-report-mcp
```
...
Your agent will call the `explainability-report-mcp` tool, which references the EU AI Act Article 13(3)(b) text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 13(3)(b)", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 13(3)(b)."
}
```
...
```python
from explainability_report_mcp import build_evidence_pack
...
- MEOK MCP: [explainability-report-mcp](https://pypi.org/project/explainability-report-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: Article 13(3)(b)
