# How to run a Fundamental Rights Impact Assessment

**Regulation:** EU AI Act (Article 27)
**MCP package:** [meok-eu-ai-act-art-26-fria-mcp](https://pypi.org/project/meok-eu-ai-act-art-26-fria-mcp/)
**Install:** `pip install meok-eu-ai-act-art-26-fria-mcp`
**Time to implement:** ~30 minutes
**Skill level:** Senior engineer / compliance lead

## Context

Filling out an FRIA template for a credit-scoring AI deployed in the EU.

This guide walks through the practical steps to implement this using the MEOK `meok-eu-ai-act-art-26-fria-mcp` MCP server. The MCP runs as a Python process; you call its tools from your agent (Claude Desktop, Cursor, Cline, or any MCP-compatible host) or via the `mcp.so` / `glama.ai` registries.

## Why this matters

The EU AI Act (Article 27) is enforced as of Article 27(4). Non-compliance is a fine of up to €15M or 2.5-3% of global annual turnover (whichever is higher) for EU AI Act. Practitioners in scope need a deterministic, audit-ready reference — not a 200-page PDF.

## Step 1: Install

```bash
pip install meok-eu-ai-act-art-26-fria-mcp
```

Or with `uv`:

```bash
uv pip install meok-eu-ai-act-art-26-fria-mcp
```
...
Your agent will call the `meok-eu-ai-act-art-26-fria-mcp` tool, which references the EU AI Act Article 27 text directly. The response is structured:

```json
{
  "risk_class": "high",
  "applicable_articles": ["Article 27", "Article 26"],
  "evidence_required": [
  ...
  "rationale": "The system processes health data of natural persons in the EU, falls within Annex III §5(a) (critical infrastructure), and meets the high-risk criteria in Article 27."
}
```
...
```python
from meok_eu_ai_act_art_26_fria_mcp import build_evidence_pack
...
- MEOK MCP: [meok-eu-ai-act-art-26-fria-mcp](https://pypi.org/project/meok-eu-ai-act-art-26-fria-mcp/)
- MEOK AI Labs: [meok.ai](https://meok.ai)
- Citation: Article 27(4)
