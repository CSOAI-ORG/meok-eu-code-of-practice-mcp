...
- **Why it can't just be copied (the Westlaw precedent):** the source frameworks are public — even Anthropic's Constitution is CC0/public-domain — but, exactly as Westlaw/LexisNexis built billion-dollar businesses organising *public* court records into proprietary navigable databases, **the defensible asset is MEOK's clause-level structural cross-mapping, not the public texts.** A competitor can read every source overnight; reproducing the structured corpus + 271-tool operationalisation still costs them the same 12–18 months. Lead time + breadth + the private master mapping + the verifiable attestation layer = the moat. *(Full argument: `DATA_ROOM/02_ip/IP_MOAT_CC0_CROSSWALK.md`.)*
