...
class BlockchainAnchor:
    ...
    def _anchor_production(self, document_hash: str) -> BlockchainProof:
        ...
        Flow:
        1. Submit hash to OTS calendar
           (default: https://alice.btc.calendar.opentimestamps.org;
            override via PATENTMCP_OTS_CALENDAR env var).
        ...
        The legacy calendar.opentimestamps.org URL is DEPRECATED by the
        OTS project; alice.btc.calendar.opentimestamps.org is the
        supported replacement. We still keep the env-var override
        mechanism so operators can pin to a specific calendar (e.g. for
        a private/whitelisted relay) without code changes.

        In production, this uses the opentimestamps library.
        """
        try:
            # opentimestamps 0.4.5+ (the venv-installed version)
            ...
            # Submit to calendar
            # NOTE: The OTS project moved from calendar.opentimestamps.org to
            # alice.btc.calendar.opentimestamps.org. We try the new URL first.
            ...
            upgraded_ts = cal.submit(ts)  # returns upgraded timestamp

            # Serialize proof
            ots_serialized = upgraded_ts.serialize()
            ots_b64 = base64.b64encode(ots_serialized).decode()

            # Extract tx hash from proof (will be a pending attestation
            # until Bitcoin block confirmation; we still get a real .ots)
            bitcoin_tx = self._extract_tx_from_ots(ots_serialized) or f"pending-{digest[:16]}"
            ...
            return BlockchainProof(
                ...
                confirmation_count=0,  # 0 = pending Bitcoin block; will be 6 after upgrade
            )
            ...
            # Network/calendar error — fall back gracefully with a log
            import logging
            logging.getLogger("patentmcp.blockchain").warning(
                "OTS production submission to alice.btc.calendar.opentimestamps.org "
                "failed (%s) — falling back to dev anchor", e,
            )
            return self._anchor_fallback(document_hash)
    ...
    def _verify_development(self, document_hash: str, proof: BlockchainProof) -> Dict[str, bool]:
        ...
        try:
            ...
            import logging
            logging.getLogger("patentmcp.blockchain").error(
                "OTS development verification failed: %s", e, exc_info=True,
            )
    ...
    def _verify_production(self, document_hash: str, proof: BlockchainProof) -> Dict[str, bool]:
        ...
        try:
            ...
            import logging
            logging.getLogger("patentmcp.blockchain").error(
                "OTS production verification failed: %s", e, exc_info=True,
            )
