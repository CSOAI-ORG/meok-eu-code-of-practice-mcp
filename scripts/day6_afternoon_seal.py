...
ts = int(time.time())
eod = (
    f"C|jeeves-cli|day6-afternoon-eod|CLOSED: Day 6 afternoon, "
    f"hive stable all 4 services 200, "
    f"mailer auto-decay hit at 05:00 — strike counter reset, "
    f"mailer tried 10 sends at 05:16 — Resend returned 10 IDs but mail.meok.ai domain still unverified, "
    f"so messages are pending in Resend queue not delivered to recipients, "
    f"Monzo email backup staged at marketing/DAY6_MONZO_EMAIL_BACKUP_2026-06-16.md, "
    f"3 keystone certs today (4EBF 42EE 5788), 3 yesterday (88F4 295B AA8A 502E 6407 3F04 92CB), "
    f"disk reclaimed to 13GB free (57%) after APFS settle, "
    f"disk reclaim cron com.meok.ops.disk-reclaim fires daily 06:00, "
    f"NEXT: Nick re-verifies mail.meok.ai in Resend (5 min) → all 10 pending + 95 staged emails deliver on next tick|{ts}"
)
s = call_tool('sigil_emit', {'line': eod})
print(f"EOD sigil: {s.get('digest')}")

# Write seal
seal = f"""# 🐉 DAY 6 AFTERNOON EOD SEAL — 16 Jun 2026 05:30 BST

_Generated {datetime.utcnow().isoformat()}Z. Day 6 afternoon closed._
...
out = Path('/Users/nicholas/clawd/DAY6_AFTERNOON_EOD_SEAL_2026-06-16.md')
out.write_text(seal)
print(f"\n✅ Day 6 afternoon EOD seal: {out}")
